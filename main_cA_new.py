...
#qsub -l procs=1,pmem=2000mb,walltime=12:00:00 -r n -N main_train_test_cv -o main_train_test_cv.out -e main_train_test_cv.err -M yifeng.li.cn@gmail.com -m bea main_cA.py
import os
...
for cell in cells:
    for wid in wids:
        ...
        for i in range(kfolds):
            ...
            alpha=0.1
            n_hidden=64
            contraction_level=0.01
            n_epochs=100
            batch_size=100

            # train, and extract features from training set
            model_trained, train_set_x_extr, training_time = cA.train_model(train_set_x_org=train_set_x_org, 
                                                                            training_epochs=n_epochs, batch_size=batch_size,
                                                                            n_hidden=n_hidden, learning_rate=learning_rate, contraction_level=contraction_level, 
            ...
            test_set_x_extr = cA.test_model(model_trained, test_set_x_org)
            valid_set_x_extr =cA.test_model(model_trained, valid_set_x_org)
